...
| Gold | Spindle | 50 | 45 | 50 | climbs, any water (ocean), flies (air 80, ~1.5x ground pace; sprint dives), fire immune, slow self-heal, rider water breathing + night vision |
...
16 / 12 feeds). Speed training raises mounted speed up to +35 %, stamina training
...
Gysahl tames do not start the clock. Gysahl alone also heals a wild-tame bird 5
and tames.
...
climbers climb walls (`onClimbable`), Flame stands on lava. Sneak submerges a water
bird; sneak-dismount only on solid ground (fliers stay mounted in the air, water-walkers
on water). The bird is not pickable for its driver unless sneaking, so clicks reach
blocks. Sneak with an empty hand opens equipment. Flight: `getFlyingSpeed` + `travel`
(hold / climb / dive / flap).
...
(`CourseSelectScreen`, `RacePayloads`); later riders of that class or higher join, each taking
